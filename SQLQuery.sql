...
select count(roll_id) rolls from customer_orders

2   .how many unique customer orders were made? 

select COUNT(distinct customer_id) from customer_orders 
3. how many successful orders were delivered by rach driver ?
...
from customer_orders
 ...
 (select *,case when cancellation 
 in('Cancellation','customer Cancellation')
  then 'c'
  else
  'nc'
  end as order_cance_deatils
from driver_order)a
where order_cance_deatils = 'nc')
group by roll_id
...
(select *,case when cancellation 
 in('Cancellation','customer Cancellation')
  ...
  from driver_order)a
  where order_cance_deatils = 'nc'))b
group by order_id
)c)d where rnk= 1



7.For each custome , how many delivered rolls had at least 1 change ans how many had no changes ?
...
,
temp_driver_order(orderid,driver_id,pickup_time,distance,duration,new_cancellation)as 

(select order_id,driver_id,pickup_time,distance,duration,
case when cancellation in ('Cancellation','Customer Cancellation')then 0 else 1 end as new_cancellation
from driver_order)
select customer_id, chg_no_chg, count(order_id) at_least_1 from
(
...
9. what was the total number of rolls orderd for each hour of the day?

select dow, count(distinct order_id) from( 
  select *, datename(dw,order_date) dow 
  from customer_orders)a
  group by dow;


  
  10  . what was the average time in minutes it tool for each driver to arrive at the fasoos HQ to pickup the other?
 ...
 ( 
  ...
  b.driver_id,b.pickup_time,b.distance,b.duration,b.cancellation ,
 ...
 11 . Is there any realtionship between the number of rolls and how long order takes to prepare?

 select order_id, COUNT(roll_id) cnt,sum(diff)/count(roll_id)times_tooks from
 (select a.order_id,a.customer_id,a.roll_id,a.not_include_items,a.extra_items_included,a.order_date,
 ...
 cast(ltrim(replace(lower(b.distance),'km','')) as decimal (4,2)) distance 
 ,b.duration,b.cancellation,
 DATEDIFF(minute,a.order_date,b.pickup_time) diff
 from customer_orders 
 a inner join 
 driver_order b
 ...
 13. what was the difference between the longest and shortest delivery Times for all orders?
...
14 . what was the average speed for each driver for each delivry and do you notice any trend for thes values?

speed=d/t

select a.order_id,a.driver_id,a.distance/a.duration speed,b.cnt from
(select order_id,driver_id,
 cast(ltrim(replace(lower(distance),'km','')) as decimal (4,2)) distance,
...
a. select *,case when LOWER(cancellation) like '%cancel%' then 0 else 1 end as can_per from driver_order 


b. select driver_id ,sum(can_per) s, count(driver_id) t
 from( 
select driver_id,case when LOWER(cancellation) like '%cancel%' 
then 0 else 1 end as can_per from driver_order
)a
group by driver_id


///////////
c .select driver_id,s*1.0/t canelled_per from
(
select driver_id ,sum(can_per) s, count(driver_id) t
 from( 
select driver_id,case when LOWER(cancellation) like '%cancel%' 
then 0 else 1 end as can_per from driver_order
)a
